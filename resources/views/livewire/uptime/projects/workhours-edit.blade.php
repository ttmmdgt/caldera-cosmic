...
new class extends Component {
    public ?ProjectWorkingHour $workingHour = null;
    ...
    #[\Livewire\Attributes\On('workhour-edit-load')]
    public function loadWorkingHour($id = null)
    {
        if (!$id) {
            return;
        }
        
        $this->workingHour = ProjectWorkingHour::find($id);
        
        if ($this->workingHour) {
            $this->project_group = $this->workingHour->project_group;
            $this->shift_id = $this->workingHour->shift_id;
            $this->work_start_time = $this->workingHour->work_start_time;
            $this->work_end_time = $this->workingHour->work_end_time;
            $this->is_working_day = $this->workingHour->is_working_day;
            $this->break_times = $this->workingHour->break_times ?? [];
        }
    }
    ...
    public function save()
    {
        ...
        try {
            ...
            $this->dispatch('close-modal', 'workhour-edit');
            $this->dispatch('workhour-updated');
            $this->reset();
            
            session()->flash('message', 'Working hour updated successfully!');
        } catch (\Exception $e) {
            session()->flash('error', 'Failed to update working hour: ' . $e->getMessage());
        }
    }
...
<x-modal name="workhour-edit" :show="false" maxWidth="3xl">
    <div class="p-6">
            ...
            <div>
                ...
                @if($workingHour)
                    <p class="text-sm text-neutral-500 mt-1">
                        Editing: {{ $workingHour->shift->name ?? 'N/A' }} for group {{ $workingHour->project_group ?? 'N/A' }}
                    </p>
                @endif
            </div>
            <button type="button" @click="$dispatch('close-modal', 'workhour-edit')" class="text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300">
        ...
        <form wire:submit="save">
            @if(!$workingHour)
                <div class="mb-4 p-4 bg-yellow-100 dark:bg-yellow-900 border border-yellow-400 dark:border-yellow-700 text-yellow-700 dark:text-yellow-300 rounded-lg">
                    <i class="icon-alert-triangle"></i> Please select a working hour to edit.
                </div>
            @endif
            ...
            <div class="space-y-4">
                ...
                <!-- Shift Selection -->
                <div>
                    <label for="shift_id" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <!-- Work Start Time -->
                    <div>
                        <label for="work_start_time" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                    ...
                    <!-- Work End Time -->
                    <div>
                        <label for="work_end_time" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                ...
                <div class="flex items-center">
                    ...
                    >
                    <label for="is_working_day" class="ml-2 block text-sm text-neutral-700 dark:text-neutral-300">
                        {{ __('Is Working Day') }}
                    </label>
                </div>
                    ...
                    @if(count($break_times) > 0)
                        <div class="space-y-2">
                            @foreach($break_times as $index => $breakTime)
                                <div wire:key="break-{{ $index }}" class="flex items-center gap-2 p-2 bg-neutral-50 dark:bg-neutral-800 rounded">
            ...
            <div class="mt-6 flex justify-end gap-3">
                ...
                <x-primary-button type="submit" wire:loading.attr="disabled" wire:target="save">
                    ...
                    <span wire:loading wire:target="save">
                        <i class="icon-loader animate-spin"></i> {{ __('Saving...') }}
                    </span>
                </x-primary-button>
            </div>
        </form>
