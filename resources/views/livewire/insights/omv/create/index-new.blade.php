...
<div id="content" class="pt-8 pb-3 max-w-7xl mx-auto sm:px-6 lg:px-8 text-neutral-800 dark:text-neutral-200">
    ...
    @else
        <div>
            Omv new with WebSocket
        </div>
        <div x-data="{ ...app(), userq: @entangle("userq").live }" x-init="
            loadRecipes()
            connectWebSocket()
        " x-on:beforeunload.window="disconnectWebSocket()">
                ...
                return {
                    ...
                    connectWebSocket() {
                        ...
                        this.socket.onopen = () => {
                            ...
                            // Request line data on connect
                            this.sendWebSocketMessage({ type: 'get-line' });
                        };
                    ...
                    handleSensorData(data) {
                        ...
                        // Handle polling A (before timer starts)
                        if (!this.timerIsRunning && data.eval) {
                            this.startTimer();
                            this.sendWebSocketMessage({ type: 'stop-polling-a' });
                            this.sendWebSocketMessage({ type: 'start-polling-b' });
                        }

                        // Handle polling B (during timer)
                    ...
                    handlePhotoData(data) {
                        ...
                        console.log(`Image captured at: ${elapsedSeconds}`);
                    ...
                    startPollingA() {
                        ...
                        // Start polling A via WebSocket
                        this.sendWebSocketMessage({ 
                            type: 'start-polling-a',
                            interval: this.pollingAInterval 
                        });
                    },
                    ...
                    startTimer() {
                        ...
                        this.sendWebSocketMessage({ type: 'get-initial-data' });
                    ...
                    startPollingB() {
                        // Stop polling A and start polling B via WebSocket
                        this.sendWebSocketMessage({ type: 'stop-polling-a' });
                        this.sendWebSocketMessage({ 
                            type: 'start-polling-b',
                            interval: this.pollingBInterval 
                        });
                    },
                    ...
                    stopTimer(isAutomatic = false) {
                        ...
                        // Stop polling B
                        this.sendWebSocketMessage({ type: 'stop-polling-b' });
