...
new #[Layout("layouts.app")] class extends Component {
    ...
    {
        $q = trim($this->q);
        $auths = InsPhDosingAuth::join("users", "ins_ph_dosing_auths.user_id", "=", "users.id")
            ->select("ins_ph_dosing_auths.*", "users.name as user_name", "users.emp_id as user_emp_id", "users.photo as user_photo")

            ->orderBy("ins_ph_dosing_auths.user_id", "desc");
...
<x-slot name="title">{{ __("Wewenang") . " — " . __("Pemantauan PDS") }}</x-slot>
<x-slot name="header">
        ...
        <div class="flex flex-col sm:flex-row gap-y-6 justify-between px-6">
            ...
            <div x-data="{ open: false }" class="flex justify-end gap-x-2">
                ...
                <div class="w-40" x-show="open" x-cloak>
                    <x-text-input-search wire:model.live="q" id="pds-q" x-ref="search" placeholder="{{ __('CARI') }}"></x-text-input-search>
                </div>
            </div>
        </div>
        <div wire:key="auth-create">
            <x-modal name="auth-create">
                <livewire:insights.pds.manage.auth-create />
            </x-modal>
        </div>
        <div wire:key="auth-edit">
            <x-modal name="auth-edit">
                <livewire:insights.pds.manage.auth-edit />
            </x-modal>
        </div>
