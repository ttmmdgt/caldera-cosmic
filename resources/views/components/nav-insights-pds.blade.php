<header class="bg-white dark:bg-neutral-800 shadow">
    <div class="flex justify-between max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="space-x-8 -my-px ml-10 flex">
            <!-- data -->
            <x-nav-link class="text-sm px-1 uppercase" href="/insights/pds/data" :active="request()->routeIs('insights.pds.data.index')" wire:navigate>
                <i class="icon-database text-sm"></i><span class="ms-3 hidden lg:inline">{{ __('Data') }}</span>
            </x-nav-link>
            <x-nav-link class="text-sm px-1 uppercase" href="{{ route('insights.pds.manage.index') }}" :active="request()->routeIs('insights.pds.manage.*')" wire:navigate>
